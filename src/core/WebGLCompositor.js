/**
 * WebGLCompositor - Blends two canvas sources with transitions
 */
export class WebGLCompositor {
     ...
     * @param {number} blend - Blend factor 0-1 (0 = full A, 1 = full B)
     * @param {string} mode - 'crossFade' | 'fadeToBlack' | 'wipe'
     */
    blend(canvasA, canvasB, blend, mode = 'crossFade') {
        ...
        gl.uniform1i(this.u_mode, modeMap[mode] || 0);
    ...
    copy(sourceCanvas) {
        ...
        this.updateTexture(this.textureA, sourceCanvas);
        ...
        // Same texture for B (blend doesn't matter when blend=0)
        gl.activeTexture(gl.TEXTURE1);
        gl.bindTexture(gl.TEXTURE_2D, this.textureA);
        gl.uniform1i(this.u_textureB, 1);

        gl.uniform1f(this.u_blend, 0);
